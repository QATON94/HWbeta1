    ...
    try:
        ...
        with open(file_path, "r", encoding="UTF-8") as file:
            if file_extension == '.json':
                ...
                transactions_reader = pd.read_excel(file_path)
...
def conv_xlsx(xslx_: Any) -> list[dict[str, dict[str, dict]]]:
    ...
    for i in range(0, num_rows):
        transactions_dict = {"operationAmount": {"currency": {}}}
        transactions_dict["id"] = xslx_.iloc[i, 0]
        transactions_dict["state"] = xslx_.iloc[i, 1]
        transactions_dict["date"] = xslx_.iloc[i, 2]
